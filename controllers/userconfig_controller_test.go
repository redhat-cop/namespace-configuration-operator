...
// pin the reconciler's wiring: the user it passes is the one the guard reads, name, labels and
// annotations included.

func UserSubject(name string, labels map[string]string, annotations map[string]string) userv1.User {
	return userv1.User{ObjectMeta: metav1.ObjectMeta{Name: name, Labels: labels, Annotations: annotations}}
}

func TestUserIsTemplateApplicable(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			template: "{{- if hasSuffix \"-admin\" .Name }}\nkind: RoleBinding\n{{- end }}",
			subject:  UserSubject("my-app-admin", nil, nil),
			expected: true,
		},
		{
			name:     "does not match hasSuffix pattern",
			template: "{{- if hasSuffix \"-admin\" .Name }}\nkind: RoleBinding\n{{- end }}",
			subject:  UserSubject("my-app-view", nil, nil),
			expected: false,
		},
		{
			name:     "matches contains pattern",
			template: "{{- if contains \"jdoe\" .Name }}\nkind: RoleBinding\n{{- end }}",
			subject:  UserSubject("team-jdoe-x", nil, nil),
			expected: true,
		},
		{
			name:     "template with no guard applies to all",
			template: "kind: RoleBinding\nmetadata:\n  name: basic",
			subject:  UserSubject("any-name", nil, nil),
			expected: true,
		},
		{
		...
		},
		{
			...
			template: "{{- if and (hasSuffix \"-admin\" .Name) (contains \"my-app\" .Name) }}\nkind: RoleBinding\n{{- end }}",
			subject:  UserSubject("my-app-admin", nil, nil),
			expected: true,
		},
		{
			name:     "and: only one condition holds",
			template: "{{- if and (hasSuffix \"-admin\" .Name) (contains \"jdoe\" .Name) }}\nkind: RoleBinding\n{{- end }}",
			subject:  UserSubject("my-app-admin", nil, nil),
			expected: false,
		},
		{
			name:     "and: no condition holds",
			template: "{{- if and (hasSuffix \"-view\" .Name) (contains \"jdoe\" .Name) }}\nkind: RoleBinding\n{{- end }}",
			subject:  UserSubject("my-app-admin", nil, nil),
			expected: false,
		},
		{
			...
			subject:  UserSubject("bdp-spark-alpha", map[string]string{"example.com/oud-group": "app-bdp-rbac-spark-alpha"}, nil),
			expected: true,
		},
		{
			name:     "hasPrefix on a label value: another family",
			template: "{{- if hasPrefix \"app-bdp-rbac-spark-\" (index .Labels \"example.com/oud-group\") }}\nkind: RoleBinding\n{{- end }}",
			subject:  UserSubject("bdp-trino-apps", map[string]string{"example.com/oud-group": "app-bdp-rbac-trino-apps"}, nil),
			expected: false,
		},
		{
			name:     "hasPrefix on a label value: label absent",
			template: "{{- if hasPrefix \"app-bdp-rbac-spark-\" (index .Labels \"example.com/oud-group\") }}\nkind: RoleBinding\n{{- end }}",
			subject:  UserSubject("plain", nil, nil),
			expected: false,
		},
		{
			name:     "truthiness of a label value: empty value does not qualify",
			template: "{{- if (index .Labels \"example.com/oud-group\") }}\nkind: RoleBinding\n{{- end }}",
			subject:  UserSubject("bdp-empty", map[string]string{"example.com/oud-group": ""}, nil),
			expected: false,
		},
		{
			name:     "ne on an annotation value",
			template: "{{- if ne (index .Annotations \"allow-pvc\") \"true\" }}\nkind: RoleBinding\n{{- end }}",
			subject:  UserSubject("quota", nil, map[string]string{"allow-pvc": "true"}),
			expected: false,
		},
		{
			name:     "ne on an annotation value: annotation absent",
			template: "{{- if ne (index .Annotations \"allow-pvc\") \"true\" }}\nkind: RoleBinding\n{{- end }}",
			subject:  UserSubject("quota", nil, nil),
			expected: true,
		},
		{
			name:     "eq on the name, the documented unrecognized example",
			template: "{{- if eq .Name \"admin\" }}\nkind: RoleBinding\n{{- end }}",
			subject:  UserSubject("dev", nil, nil),
			expected: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := reconciler.isTemplateApplicableToUser(apis.LockedResourceTemplate{ObjectTemplate: tt.template}, tt.subject)
...
func TestUserFilterApplicableTemplates(t *testing.T) {
	...
	t.Run("keeps the matching guarded template and the unconditional one", func(t *testing.T) {
		...
		filtered := reconciler.filterApplicableTemplates(templates, UserSubject("my-app-admin", nil, nil))
		if len(filtered) != 2 {
	...
	t.Run("returns an empty slice when no template matches", func(t *testing.T) {
		...
		filtered := reconciler.filterApplicableTemplates(templates, UserSubject("my-app-view", nil, nil))
		if len(filtered) != 0 {
			t.Errorf("Expected 0 templates, got %d", len(filtered))
		}
	})

	t.Run("a label-guarded template is skipped for a user outside its family", func(t *testing.T) {
		templates := []apis.LockedResourceTemplate{
			{ObjectTemplate: "{{- if hasPrefix \"app-bdp-rbac-spark-\" (index .Labels \"example.com/oud-group\") }}\nkind: RoleBinding\n{{- end }}"},
		}
		filtered := reconciler.filterApplicableTemplates(templates, UserSubject("bdp-trino-apps", map[string]string{"example.com/oud-group": "app-bdp-rbac-trino-apps"}, nil))
		if len(filtered) != 0 {
			t.Errorf("Expected 0 templates, got %d", len(filtered))
		}
	})
